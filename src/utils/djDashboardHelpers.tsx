...
/**
 * Convert numeric key + mode (0 minor, 1 major) into Camelot notation
 * Returns null when data is missing or out of range
 */
export const camelotKeyFromAudio = (key?: number | null, mode?: number | null): string | null => {
  if (key === null || key === undefined || key < 0 || key > 11) {
    return null;
  }
  ...
  if (mode === 1) {
    return majorMap[key] ?? null;
  }

  // Default to minor map (mode 0 or undefined treated as minor)
  return minorMap[key] ?? null;
};
...
export const getSourceBadge = (source: string): JSX.Element | null => {
  switch (source) {
    case 'ai':
      return <Badge className="bg-[var(--neon-purple)]/30 border-[var(--neon-purple)] font-[Inter] text-[11px] text-[rgb(215,72,255)] text-[var(--neon-purple)] text-xs">
        <Sparkles className="mr-1 w-3 h-3" />
        AI
      </Badge>;
    case 'spotify':
      return <Badge className="bg-gradient-to-r from-green-500 to-green-600 border-0 text-white text-xs">
        <Music className="mr-1 w-3 h-3" />
        Spotify
      </Badge>;
    case 'apple':
      return <Badge className="bg-gradient-to-r from-gray-600 to-gray-700 border-0 text-white text-xs">
        <Music className="mr-1 w-3 h-3" />
        Apple
      </Badge>;
    case 'tip-request':
      return <Badge className="bg-gradient-to-r from-[var(--neon-yellow)]/20 to-[var(--neon-yellow)]/10 border border-[var(--neon-yellow)]/30 text-[var(--neon-yellow)] text-xs">
        <Zap className="mr-1 w-3 h-3" />
        Guest Suggestion
      </Badge>;
    case 'hidden-anthems':
      return <Badge className="bg-gradient-to-r from-[var(--neon-pink)]/20 to-[var(--neon-pink)]/10 border border-[var(--neon-pink)]/30 text-[var(--neon-pink)] text-xs">
        <Radio className="mr-1 w-3 h-3" />
        Hidden Anthem
      </Badge>;
    default:
      return null;
